...
def wishMe():
    ...
    if(hour>=0) and (hour<=12) and ('AM' in t):
        speak(f"Good morning Abdallah, it's {day} and the time is {t}")
    elif(hour>=12)  and (hour<=16) and ('PM' in t):
        speak(f"Good afternoon Abdallah, it's {day} and the time is {t}")
    else:
        speak(f"Good evening Abdallah, it's {day} and the time is {t}")
...
def openApp(command):
    if "calculator" in command:
        ...
        os.startfile('C:\\Users\\d\\AppData\\Roaming\\Microsoft\\Windows\\Start Menu\\Programs\\Visual Studio Code\\Visual Studio Code')
...
def closeApp(command):
    if "calculator" in command:
        ...
        speak("closing visual")
        os.system('taskkill /f /im Visual Studio Code')
...
if __name__ == "__main__":
       ...
       while True:
        ...
        if ('facebook' in query) or ('discord' in query) or ('whatsapp' in query) or ('instagram' in query):
            ...
            speak("Volume muted")
        elif ("open calculator" in query) or ("open notepad" in query) or ("open paint" in query) or ("open visual" in query):
            openApp(query)
        elif ("close calculator" in query) or ("close notepad" in query) or ("close paint" in query) or ("close visual" in query):
            closeApp(query)
